...
RSpec.describe Project, type: :model do
  ...
  it 'persists and supports mass assignment' do
    project = Project.create(name: 'Mon Projet', description: 'Ma description', location: "Mon endroit", category_id: 1, amount: 10000)
    project.reload

    expect(project.name).to         eq 'Mon Projet'
    expect(project.description).to  eq 'Ma description'
    expect(project.location).to     eq 'Mon endroit'
